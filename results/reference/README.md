...
The community benchmark JSON files retain their original bytes and run identifiers. The separately labeled Mia summary is a transcription of published figures.
...
- `mia-reported-prose.json`: Mia's three-Spark SGLang prose summary, transcribed from README at `e59e6eb`; no matching raw per-request results were available in the reviewed snapshot.
- `upstream-check-2026-09-11.json`: checked heads, Tony's two new commits, changed-file list and hashes proving the existing reference benchmark/source files are unchanged.
