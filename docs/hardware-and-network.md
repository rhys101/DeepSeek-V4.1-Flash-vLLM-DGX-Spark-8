...
During the recorded benchmark, twenty samples per GPU showed SM clocks of 2,171–2,190 MHz, P0 and no active reported throttle flags. These observations do not prove the absence of performance-state changes between samples. GPU utilization can include time waiting in collectives; GPU power is not total node power.

The model-unloaded GPU-state probe completed separately on all eight nodes without a large collapse. It does not establish the resident model’s state throughout serving. [Raw clock and probe evidence](../results/2026-09-10/).
