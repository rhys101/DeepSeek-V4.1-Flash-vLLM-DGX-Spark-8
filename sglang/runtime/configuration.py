...
def load(path):
    ...
    if c['context_length'] != 300000 or c['max_running_requests'] != 8 or c['draft_length'] != 5:
        raise ValueError('This comparison profile is fixed at 300K context, eight requests and Mia’s DSpark draft length 5')
    ...
    if not 2400000 <= c['max_total_tokens'] <= 3200000: raise ValueError('Unexpected KV pool size')
