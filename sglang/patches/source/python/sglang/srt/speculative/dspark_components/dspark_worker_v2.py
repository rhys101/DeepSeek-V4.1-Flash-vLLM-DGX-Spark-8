...
class DSparkWorkerV2(BaseSpecWorker):
    def __init__(
        ...
        if self._draft_is_moe:
            methods = {}
            for role, model in (
                ("target", self.target_worker.model_runner.model),
                ("draft", self.draft_model),
            ):
                methods[role] = {
                    name: dict(
                        method=type(module.quant_method).__name__,
                        local_experts=module.num_local_experts,
                        intermediate=module.intermediate_size_per_partition,
                        ep_size=module.moe_ep_size, ep_rank=module.moe_ep_rank,
                        tp_size=module.moe_tp_size, tp_rank=module.moe_tp_rank,
            ...
            logger.info("DSPARK_MOE_LAYOUT %s", json.dumps(methods, sort_keys=True))
    ...
    def _draft_context(self):
        # Construction, capture and eager/replay execution must all select
        # the same draft MoE backend, then restore the target backend.
        parallel_context = (
            draft_tp_context(get_parallel().attn_tp_group)
            if self._draft_dp_context_enabled
            else nullcontext()
        )
        with parallel_context, speculative_moe_backend_context():
            yield
