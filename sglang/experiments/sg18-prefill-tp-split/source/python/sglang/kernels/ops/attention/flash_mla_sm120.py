...
def _split_kv_pages_to_64(
    ...
    dst pages remain zero-initialized because FlashInfer may gather slot 0 for
    masked candidates.
    """
    ...
    if buf is None or buf.shape[0] < num_dst_pages:
        ...
        # FlashInfer 0.6.18 gathers slot 0 for masked (-1) candidates.
        # Keep untouched scratch finite so zero probability cannot multiply NaN.
        # One initialization per allocation/grow; touched-page copying is unchanged.
