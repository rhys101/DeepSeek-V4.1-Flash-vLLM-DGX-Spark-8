...
from sglang.srt.layers.attention.dsv4.spark_prefill_dense import NativePrefillTPSplit
...
def two_level_decode_logits(
    ...
    *,
    ...
    if (
        ...
        from sglang.kernels.ops.attention.dsv4.spark_candidate_blocks import candidate_block_logits_bounded
        return candidate_block_logits_bounded(
            logits,
            seq_lens,
...
class DeepseekV4AttnBackend(
    ...
    def __init__(
        ...
        self.native_prefill_tp = NativePrefillTPSplit.from_parallel(
    ...
    def _low_ratio_index_topk_dense(
        ...
        if self.native_prefill_tp.eligible(
            num_tokens, max(lc_per_req),
            prefill=forward_batch.forward_mode.is_extend_without_speculative(),
            device=device,
        ):
            selected, masks = self.native_prefill_tp.select(
                indexer, q_fp4, q_sf, k_fp4, k_sf, weights, ks, compress_lens,
                lc_per_req, q_lens_cpu, scorer=_dense_fp4_mqa_logits,
                topk=topk_transform_ragged_v2, mask_topk=_mask_topk_scores,
                consume=self.candidate_masks if indexer.uses_candidates else None,
                layer_id=layer.layer_id, ratio=ratio,
            )
            if indexer.is_candidate_source:
